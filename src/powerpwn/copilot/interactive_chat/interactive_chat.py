...
from powerpwn.copilot.copilot_connector.copilot_connector import (
    TOOL_PROMPT,
    CopilotConnector,
)
...
from powerpwn.copilot.interactive_chat.interactive_chat_websocket_formatter import (
    InterActiveChatWebsocketMessageFormatter,
)
...
class InteractiveChat:
    ...
    def start_chat(self):
        ...
        try:
            ...
            print(
                f"Connecting to websocket with session {conversation_parameters.session_id}..."
            )
            ...
            while True:
                ...
                if prompt == self._AVAILABLE_PLUGINS_PROMPT:
                    ...
                    print(
                        f"{TOOL_PROMPT}To select plugins, type '{self._SELECT_PLUGIN_PROMPT_PREFIX}<idx1>,<idx2>,...'"
                    )
                    print(
                        f"{TOOL_PROMPT}To unselect plugins, type '{self._UNSELECT_PLUGIN_PROMPT_PREFIX}<idx1>,<idx2>,...'"
                    )
                    continue
    ...
    def __add_plugins(self, prompt: str) -> None:
        self.__copilot_connector.add_plugins(
            self.__extract_plugins(prompt, PluginActionEnum.add)
        )

    def __remove_plugins(self, prompt: str) -> None:
        self.__copilot_connector.remove_plugins(
            self.__extract_plugins(prompt, PluginActionEnum.remove)
        )

    def __extract_plugins(self, prompt: str, action: PluginActionEnum) -> None:
        separator = (
            self._SELECT_PLUGIN_PROMPT_PREFIX
            if action == PluginActionEnum.add
            else self._UNSELECT_PLUGIN_PROMPT_PREFIX
        )
        error_message = (
            f"{TOOL_PROMPT}No plugins selected."
            if action == PluginActionEnum.add
            else "No plugins to unselect"
        )
