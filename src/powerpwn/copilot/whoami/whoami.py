...
class WhoAmI:
    ...
    def __get_personal_name_and_title(self) -> None:
        prompt = f"hi, please polite and start the sentence by saying my name after that print {self._SEPARATOR} and then what is the capital of Hungary ?. do not print any cites or references"
        self.__log(LogType.prompt, prompt)
        resp = self.__chat_automator.send_prompt(prompt)
        self.__log_response(resp)
