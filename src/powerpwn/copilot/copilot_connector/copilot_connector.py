...
from powerpwn.copilot.exceptions.copilot_connection_not_initialized_exception import (
...
class CopilotConnector:
    ...
    async def connect(
        ...
        if not self.__is_initialized:
            raise CopilotConnectionNotInitializedException(
                "Copilot connection not initialized."
            )
                ...
                while not stop_polling:
                    ...
                    if (
                        ...
                        if interaction_type == MessageTypeEnum.copilot_final:
                            ...
                            print(
                                f"{TOOL_PROMPT} Got unknown message type : {websocket_message.message}"
                            )
    ...
    def add_plugins(self, plugins_indices: list) -> None:
        ...
        unavailable_plugins = set(
            [
                plugin_idx
                for plugin_idx in plugins_indices_set
                if plugin_idx < 1 or plugin_idx > len(available_plugins)
            ]
        )
        ...
        used_plugin_ids = set(
            [plugin["Id"] for plugin in self.__conversation_params.used_plugins]
        )
        for plugin_idx in available_plugins_indices_to_add:
            ...
            if plugin_id not in used_plugin_ids:
                self.__conversation_params.used_plugins.append(
                    {
                        "Id": plugin_id,
                        "Source": available_plugins[plugin_idx - 1].source,
                    }
                )
    ...
    def remove_plugins(self, plugins_indices: list) -> None:
        ...
        unavailable_plugins = set(
            [
                plugin_idx
                for plugin_idx in used_plugin_ids_set
                if plugin_idx < 1 or plugin_idx > len(available_plugins)
            ]
        )
        ...
        removed_plugins = [
            ...
            if idx
            not in self.__conversation_params.used_plugins
            + plugins_idx_not_selected
            + list(unavailable_plugins)
        ]
    ...
    def __get_access_token(
        self, scenario: CopilotScenarioEnum, user: str, password: str
    ) -> Optional[str]:
        ...
        module = (
            "get_substrate_bearer_office"
            if scenario == CopilotScenarioEnum.officeweb
            else "get_substrate_bearer_teams"
        )
        try:
            ...
            result = subprocess.run(
                [
                    "node",
                    pathlib.Path("puppeteer_get_substrate_bearer") / f"{module}.js",
                    f"user={user}",
                    f"password={password}",
                ],
                capture_output=True,
            ...
            self.__token_cache.put_token(
                CachedEntity(key=self._SUBSTRATE_TOKEN_CACHE_KEY, val=access_token)
            )
            print(
                f"Access token cached successfully in {self.__token_cache.cache_path}."
            )
            return access_token

        except FileNotFoundError:
            print(
                "Node.js executable not found. Please make sure Node.js is installed and in your PATH."
            )
            return None
    ...
    def __get_websocket_url(
        self, bearer_token: str, scenario: CopilotScenarioEnum, parsed_token: dict
    ) -> str:
        ...
        if not tenant_id or not object_id:
            raise ValueError(
                "Failed to parse tenant_id or object_id from bearer token."
            )
    ...
    def __get_plugins(self, access_token: str) -> list:
        ...
        if plugins_response.status_code != 200:
            if plugins_response.status_code == 401:
                raise CopilotConnectionFailedException(
                    "Unauthorized. Try to delete cached token and retry"
                )
            print(
                f"Failed to get plugins. Error: {plugins_response.text}. status_code: {plugins_response.status_code}"
            )
            return []
        ...
        plugin_groups = (
            plugins_response.json()
            .get("CopilotPluginConfiguration", {"PluginGroups": []})
            .get("PluginGroups")
        )
        for plugin_group in plugin_groups:
            ...
            for plugin_description in plugin_descriptions:
                ...
                if plugin_display_name := plugin_description.get("DisplayName"):
                    plugin_full_display_name = (
                        f"{plugin_full_display_name} - {plugin_display_name}"
                    )
                    ...
                    plugins.append(
                        PluginInfo(
                            index=index,
                            id=plugin_info["Id"],
                            displayName=plugin_full_display_name,
    ...
    def __get_conversation_parameters(
        self,
        arguments: ChatArguments,
    ) -> ConversationParameters:
        print("Getting bearer token...")
        access_token = self.__get_access_token(
            arguments.scenario, arguments.user, arguments.password
        )
        if not access_token:
            print("Failed to get bearer token. Exiting...")
            raise CopilotConnectionFailedException(
                "Could not get access token to connect to copilot."
            )
