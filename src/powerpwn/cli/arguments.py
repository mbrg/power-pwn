...
def module_dump(sub_parser: argparse.ArgumentParser):
    ...
    dump_parser.add_argument("-l", "--log-level", default=logging.INFO, type=lambda x: getattr(logging, x), help="Configure the logging level.")
...
def module_nocodemalware(command_subparsers: argparse.ArgumentParser):
    ...
    nocodemalware_parser.add_argument("-w", "--webhook-url", required=True, type=str, help="Webhook url to the flow factory installed in powerplatform")
    nocodemalware_parser.add_argument("-l", "--log-level", default=logging.INFO, type=lambda x: getattr(logging, x), help="Configure the logging level.")
...
def module_nocodemalware_subcommand_exec(command_subparsers: argparse.ArgumentParser):
    ...
    execute_command_parser.add_argument("-t", "--type", required=True, type=str, choices=[cmd_type.value for cmd_type in CodeExecTypeEnum], help="Command type")
    ...
    ransomware_parser.add_argument("-k", "--encryption-key", required=True, type=str, help="an encryption key used to encrypt each file identified (AES256)")
...
def module_backdoor(command_subparsers: argparse.ArgumentParser):
    ...
    backdoor_parser.add_argument("-w", "--webhook-url", required=True, type=str, help="Webhook url to the flow factory installed in powerplatform")
    backdoor_parser.add_argument("-l", "--log-level", default=logging.INFO, type=lambda x: getattr(logging, x), help="Configure the logging level.")
    ...
    ## Delete Flow parser ##
    delete_flow_parser = backdoor_subparsers.add_parser(BackdoorActionType.delete_flow.value, description="Deletes flow.", help="Deletes flow using installed backdoor flow.")
